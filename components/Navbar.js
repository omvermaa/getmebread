...
import { fetchuser, searchUsers } from "@/actions/useractions";
import {
  UserIcon,
  SettingsIcon,
  LockIcon,
...
const Navbar = () => {
  ...
  // ✅ FIX: Add Mounted State to prevent Hydration Errors
    ...
    <nav className="fixed w-full z-50 top-0 start-0 border-b border-white/10 bg-slate-950/70 backdrop-blur-md">
      <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between px-4 py-3 gap-4">
        ...
        <div className="flex-1 max-w-md relative" ref={searchRef}>
          {/* ✅ FIX: Only render input after client mount */}
          {isMounted ? (
            ...
            // Optional: Render a skeleton or empty div to prevent layout shift during load
            <div className="w-full h-10 bg-slate-900/50 rounded-full border border-slate-700"></div>
          )}
            ...
            <div className="relative">
              ...
              >
                <div className="relative w-7 h-7 overflow-hidden rounded-full ring-2 ring-purple-500/50 group-hover:ring-purple-500 transition-all">
                  <Image
                    src={user?.profilePic || session?.user?.image || "/avatar.gif"}
                    alt="User"
                    fill
              ...
              <div
                className={`absolute right-0 mt-3 w-64 bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl shadow-black/50 transform origin-top-right transition-all duration-200 ease-out ${showDropDown
